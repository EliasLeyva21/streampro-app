...
const ADMIN_EMAIL = 'forex3339@gmail.com'; 
      ...
      <div className="w-full max-w-md z-10 animate-in fade-in zoom-in duration-700">
        <div className="bg-white/[0.02] backdrop-blur-2xl p-8 md:p-12 rounded-[2.5rem] md:rounded-[3.5rem] border border-white/10 shadow-2xl flex flex-col items-center text-center">
          ...
          <div className="mt-2 pt-6 border-t border-white/5 w-full">
            ...
            <a href="https://wa.me/51902257451?text=Hola,%20deseo%20adquirir%20un%20plan%20mensual%20del%20sistema%20ZERO." target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-white transition-colors text-[10px] md:text-xs font-black italic uppercase tracking-widest flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen bg-[#050509] flex flex-col items-center justify-center p-4 text-center text-white">
      <div className="bg-red-500/10 border border-red-500/20 p-8 md:p-12 rounded-[2.5rem] md:rounded-[3.5rem] max-w-lg shadow-[0_0_50px_rgba(239,68,68,0.1)] animate-in fade-in zoom-in duration-500">
        ...
        <a href="https://wa.me/51902257451?text=Hola,%20deseo%20renovar%20mi%20suscripción%20al%20sistema%20ZERO%20para%20desbloquear%20mi%20panel." target="_blank" rel="noopener noreferrer" className="w-full block bg-emerald-600 hover:bg-emerald-500 text-white py-4 md:py-5 rounded-2xl font-black uppercase italic tracking-widest transition-all shadow-xl mb-4 text-xs md:text-sm">
          Contactar Soporte / Pagar
        </a>
        <button onClick={onLogout} className="text-slate-500 hover:text-white font-bold text-[10px] md:text-xs uppercase tracking-widest flex items-center justify-center gap-2 w-full mt-6">
...
function App() {
  ...
  const [form, setForm] = useState({ 
    nombre: '', whatsapp: '', servicio: '', customServicio: '', monto: '', vencimiento: '', correo: '', contrasena: '', url: ''
  });
  
  const [editInvId, setEditInvId] = useState(null);
  const [invForm, setInvForm] = useState({ 
    servicio: '', customServicio: '', correo: '', contrasena: '', proveedor_nombre: '', proveedor_whatsapp: '', costo: '', vencimiento: '', nombre: '', url: '' 
  });
  ...
  const verificarSuscripcionSaaS = useCallback(async (user) => {
    try {
      let { data, error } = await supabase.from('usuarios_sistema').select('*').eq('user_id', user.id).maybeSingle();
      if (error) console.error("Error buscando usuario:", error);

      if (!data) {
        const isAdmin = user.email === ADMIN_EMAIL;
        const newProfile = { user_id: user.id, email: user.email, rol: isAdmin ? 'admin' : 'cliente', estado: 'activo' };
        const { data: insertedData, error: insertError } = await supabase.from('usuarios_sistema').insert([newProfile]).select().single();
        if (insertError) console.error("Error insertando usuario en usuarios_sistema:", insertError);
        data = insertedData;
      }
      
      if (data) {
        data.rol = data.rol?.replace(/['"]/g, '');
        data.estado = data.estado?.replace(/['"]/g, '');
      }

      setSystemUser(data);
      if (data && (data.estado === 'activo' || data.rol === 'admin')) {
        cargarTodo(user.id);
  ...
  const cargarUsuariosSaaS = async () => {
    ...
    if (data) {
      const cleanData = data.map(u => ({
        ...u,
        rol: u.rol?.replace(/['"]/g, ''),
        estado: u.estado?.replace(/['"]/g, '')
      }));
      setListaUsuariosSaaS(cleanData);
    }
  };
  ...
  // --- VARIABLES FINANCIERAS (USE MEMO) ---
  const finanzasUnificadas = useMemo(() => {
    const ingresos = ventasHistoricas.map(v => ({ 
      id: `v-${v.id}`, tipo: 'ingreso', fecha: v.fecha_pago || v.created_at || new Date().toISOString(), 
      titulo: v.proveedores?.nombre_negocio || 'Venta Rápida', servicio: v.proveedores?.servicio || 'Varios', monto: v.monto || 0 
    }));
    
    const gastos = inventario.map(i => ({ 
      id: `i-${i.id}`, tipo: 'gasto', fecha: i.created_at || new Date().toISOString(), 
      titulo: i.proveedor_nombre ? `Prov: ${i.proveedor_nombre}` : 'Compra Stock', servicio: i.servicio || 'Varios', monto: i.costo_total || 0 
    }));
  ...
  const enviarWhatsApp = (cliente) => {
    ...
    const mensaje = `*ZERO - RECORDATORIO DE PAGO*%0A%0AHola *${cliente.nombre_negocio}*, te recordamos que tu servicio de *${cliente.servicio}* ${estadoMsg} (*${new Date(cliente.fecha_vencimiento).toLocaleDateString()}*).%0A%0A*Monto a renovar:* S/. ${cliente.monto.toFixed(2)}%0A%0APor favor, envíanos el comprobante para asegurar la continuidad de tu servicio. ¡Gracias!`;

    window.open(`https://wa.me/${cliente.whatsapp}?text=${mensaje}`, '_blank');
  };

  // NUEVO: Función para Enviar ACCESOS por WhatsApp
  const enviarAccesosWhatsApp = (cliente) => {
    if (!cliente.whatsapp) {
      alert("Este cliente no tiene un número de WhatsApp registrado.");
      return;
    }
    
    let mensaje = `*TUS ACCESOS - SERVICIO*%0A%0A` +
                  `Hola *${cliente.nombre_negocio}*, aquí tienes los detalles de tu cuenta de *${cliente.servicio}*:%0A%0A` +
                  `*Correo:* ${cliente.correo || 'No registrado'}%0A` +
                  `*Contraseña:* ${cliente.contrasena || 'No registrada'}%0A`;
                  
    if (cliente.url && (cliente.servicio === 'IPTV' || cliente.servicio === 'Magis TV')) {
      mensaje += `*URL de Acceso:* ${cliente.url}%0A`;
    }

    mensaje += `%0A*Vencimiento:* ${new Date(cliente.fecha_vencimiento).toLocaleDateString()}%0A%0A` +
  ...
  const enviarWhatsAppProveedor = (item) => {
    if (!item.proveedor_whatsapp) return alert("No has registrado el WhatsApp de este proveedor.");
  ...
  const handleGuardarCliente = async (e) => {
    ...
    const payload = { 
      ...
      url: (servicioFinal === 'IPTV' || servicioFinal === 'Magis TV') ? form.url : null
    };
  ...
  const handleRenovacionStock = async (i) => {
    if(!i.fecha_vencimiento) return alert("Esta cuenta no tiene fecha de vencimiento configurada.");
    const f = new Date(i.fecha_vencimiento); f.setMonth(f.getMonth() + 1);
    const { error } = await supabase.from('inventario').update({ fecha_vencimiento: f.toISOString().split('T')[0] }).eq('id', i.id);
    if (error) alert(error.message); else cargarTodo(session.user.id);
  };
  ...
  const handleGuardarStock = async (e) => {
    ...
    const payload = { 
      ...
      user_id: session.user.id,
      nombre: (servicioFinal === 'IPTV' || servicioFinal === 'Magis TV') ? invForm.nombre : null,
      url: (servicioFinal === 'IPTV' || servicioFinal === 'Magis TV') ? invForm.url : null
    };

    let result = editInvId ? await supabase.from('inventario').update(payload).eq('id', editInvId) : await supabase.from('inventario').insert([payload]);
    if (result.error) alert("Error al guardar: " + result.error.message); else { setIsInvModalOpen(false); cargarTodo(session.user.id); }
  };

  const handleGuardarNombre = () => { localStorage.setItem('zero_user_name', userName); alert("¡Actualizado!"); };
  
  const handleActualizarPassword = async () => {
    if (newPassword.length < 6) return alert("Mínimo 6 caracteres.");
    setUpdatingPass(true);
    const { error } = await supabase.auth.updateUser({ password: newPassword });
    if (error) alert("Error: " + error.message); else { alert("¡Éxito!"); setNewPassword(''); }
  ...
  const cerrarModal = () => { 
    ...
    setForm({ nombre: '', whatsapp: '', servicio: '', customServicio: '', monto: '', vencimiento: '', correo: '', contrasena: '', url: '' }); 
  };
  const cerrarInvModal = () => { 
    setIsInvModalOpen(false); setEditInvId(null); 
    setInvForm({ servicio: '', customServicio: '', correo: '', contrasena: '', proveedor_nombre: '', proveedor_whatsapp: '', costo: '', vencimiento: '', nombre: '', url: '' }); 
  };
  ...
  const rolSeguro = systemUser?.rol?.replace(/['"]/g, '');
  const estadoSeguro = systemUser?.estado?.replace(/['"]/g, '');
  
  if (estadoSeguro === 'suspendido' && rolSeguro !== 'admin') return <SuspendedView onLogout={() => supabase.auth.signOut()} />;
    ...
    <div className="min-h-screen bg-[#050509] text-slate-300 flex font-sans overflow-x-hidden select-none">
      ...
      <main className="flex-1 h-screen overflow-y-auto px-5 py-8 lg:px-12 lg:py-10 relative bg-[#080811] custom-scrollbar w-full">
        ...
        <button 
          ...
          className={`fixed z-[60] transition-all shadow-2xl backdrop-blur-md flex items-center justify-center bottom-6 right-6 w-14 h-14 rounded-full bg-blue-600 text-white border border-white/20 hover:scale-105 active:scale-95 lg:bottom-auto lg:right-auto lg:top-14 lg:w-auto lg:h-auto lg:p-3 lg:rounded-2xl lg:bg-white/5 lg:text-slate-500 lg:border-white/10 lg:hover:text-white lg:hover:bg-white/10 ${isSidebarOpen ? 'lg:left-80' : 'lg:left-10'}`}
        >
        ...
        {/* SAAS ADMIN */}
        {vistaActual === 'saas_admin' && rolSeguro === 'admin' && (
          <div className="animate-in fade-in duration-700 max-w-7xl mx-auto pb-20 lg:pb-0">
             ...
             <div className="bg-white/[0.02] border border-white/5 p-6 lg:p-8 rounded-[2rem] lg:rounded-[3rem]">
                ...
                <div className="space-y-4">
                  {listaUsuariosSaaS.length === 0 && <p className="text-slate-500 italic text-sm">No hay usuarios visibles. Revisa que RLS esté desactivado en Supabase.</p>}
                  {listaUsuariosSaaS.map(user => (
                    <div 
                      key={user.user_id} 
                      className={`p-5 lg:p-6 rounded-[1.5rem] lg:rounded-3xl border flex flex-col md:flex-row justify-between md:items-center gap-4 md:gap-0 ${user.estado === 'activo' ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-red-500/20 bg-red-500/5'}`}
                    >
                      <div>
                      ...
                      <div className="flex flex-wrap gap-2 lg:gap-4 w-full md:w-auto">
                        {user.estado === 'suspendido' ? 
                          <button 
                            onClick={() => modificarEstadoSaaS(user.user_id, 'activo')} 
                            className="flex-1 md:flex-none justify-center items-center gap-2 bg-emerald-600/20 text-emerald-400 px-4 py-3 md:py-2 rounded-xl text-xs font-bold uppercase hover:bg-emerald-600/40 flex"
                          >
                            <CheckCircle2 size={16}/> Activar
                          </button> 
                          :
                          <button 
                            onClick={() => modificarEstadoSaaS(user.user_id, 'suspendido')} 
                            className="flex-1 md:flex-none justify-center items-center gap-2 bg-red-600/20 text-red-400 px-4 py-3 md:py-2 rounded-xl text-xs font-bold uppercase hover:bg-red-600/40 flex"
                          >
                            <AlertOctagon size={16}/> Suspender
                          </button>
                        }
                        <button 
                          onClick={() => renovarSuscripcionCRM(user.user_id, 30)} 
                          className="flex-1 md:flex-none justify-center items-center gap-2 bg-blue-600/20 text-blue-400 px-4 py-3 md:py-2 rounded-xl text-xs font-bold uppercase hover:bg-blue-600/40 flex"
                        >
                ...
                <div className="col-span-12 lg:col-span-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4 lg:gap-4">
                    <div className="bg-blue-500/10 border border-blue-500/20 p-5 lg:p-6 rounded-3xl shadow-[0_0_20px_rgba(59,130,246,0.05)] flex-1">
                      <div className="flex justify-between items-start mb-2">
                        <p className="text-blue-400 text-[9px] lg:text-[10px] font-black uppercase italic tracking-widest">Ingresos Brutos</p>
                    ...
                    <div className="bg-violet-500/10 border border-violet-500/20 p-5 lg:p-6 rounded-3xl shadow-[0_0_20px_rgba(139,92,246,0.05)] flex-1">
                      <div className="flex justify-between items-start mb-2">
                        <p className="text-violet-400 text-[9px] lg:text-[10px] font-black uppercase italic tracking-widest">Utilidad Neta</p>
                    ...
                    <div className="bg-emerald-500/10 border border-emerald-500/20 p-5 lg:p-6 rounded-3xl shadow-[0_0_20px_rgba(16,185,129,0.05)] flex-1">
                      <div className="flex justify-between items-start mb-2">
                        <p className="text-emerald-400 text-[9px] lg:text-[10px] font-black uppercase italic tracking-widest">Clientes Activos</p>
                    ...
                    <div className="bg-red-500/10 border border-red-500/20 p-5 lg:p-6 rounded-3xl shadow-[0_0_20px_rgba(239,68,68,0.05)] flex-1">
                      <div className="flex justify-between items-start mb-2">
                        <p className="text-red-400 text-[9px] lg:text-[10px] font-black uppercase italic tracking-widest">Atención Urgente</p>
            ...
            <div className="animate-in fade-in duration-500 max-w-7xl mx-auto pb-20 lg:pb-0">
              <header className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6 pt-2 lg:pt-4">
                <div className="flex items-center gap-4">
                  <div className="w-0 lg:w-16"></div>
                  <h2 className="text-4xl lg:text-5xl font-black text-white italic uppercase tracking-tighter">Clientes</h2>
                </div>
                ...
                {registrosFiltrados.map(item => {
                  ...
                  return (
                    <div key={item.id} className="bg-white/[0.02] border border-white/5 p-6 lg:p-8 rounded-[2rem] lg:rounded-[3rem] hover:border-blue-500/30 transition-all group flex flex-col h-full">
                      ...
                      {/* Info Adicional del Cliente (Correos, Pass, URL) si existe */}
                      {(item.correo || item.contrasena || item.url) && (
                        <div className="bg-black/40 rounded-[1.5rem] lg:rounded-2xl p-4 mb-5 lg:mb-6 space-y-3 border border-white/5">
                          {item.correo && (
                            <div className="flex justify-between items-center gap-2">
                              <div className="truncate flex-1"><p className="text-[8px] lg:text-[9px] text-slate-500 uppercase font-black tracking-widest">Correo</p><p className="text-xs lg:text-sm font-bold text-slate-200 truncate" title={item.correo}>{item.correo}</p></div>
                              <button onClick={() => copiarAlPortapapeles(item.correo)} className="text-slate-600 hover:text-white p-2 transition-colors flex-shrink-0"><Copy size={14}/></button>
                            </div>
                          )}
                          {item.contrasena && (
                            <div className="flex justify-between items-center gap-2">
                              <div className="truncate flex-1"><p className="text-[8px] lg:text-[9px] text-slate-500 uppercase font-black tracking-widest">Contraseña</p><p className="text-xs lg:text-sm font-bold text-slate-200 truncate" title={item.contrasena}>{item.contrasena}</p></div>
                              <button onClick={() => copiarAlPortapapeles(item.contrasena)} className="text-slate-600 hover:text-white p-2 transition-colors flex-shrink-0"><Copy size={14}/></button>
                            </div>
                          )}
                          {item.url && (
                            <div className="flex justify-between items-center gap-2">
                              <div className="truncate flex-1"><p className="text-[8px] lg:text-[9px] text-slate-500 uppercase font-black tracking-widest">URL Acceso</p><p className="text-xs lg:text-sm font-bold text-slate-200 truncate" title={item.url}>{item.url}</p></div>
                              <button onClick={() => copiarAlPortapapeles(item.url)} className="text-slate-600 hover:text-white p-2 transition-colors flex-shrink-0"><Copy size={14}/></button>
                            </div>
                          )}
                        </div>
                      )}
                      ...
                      <div className="flex items-center justify-between border-t border-white/5 pt-5 lg:pt-6 mt-auto">
                         <div className="space-y-1">
                            ...
                            <div className="text-[8px] lg:text-[9px] font-bold text-slate-600 uppercase">
                              {dias === 0 ? "¡Hoy!" : dias > 0 ? `Faltan ${dias} d` : `Venció hace ${Math.abs(dias)} d`}
                            </div>
                         </div>
                         
                         <div className="flex gap-1 lg:gap-2">
                            {/* NUEVO BOTON: ENVIAR ACCESOS */}
                            <button 
                              onClick={() => enviarAccesosWhatsApp(item)} 
                              className="p-2 transition-all drop-shadow-lg text-blue-400 hover:text-blue-300 hover:scale-110 lg:hover:scale-125" 
                              title="Enviar Accesos"
                            >
                              <Key size={18}/>
                            </button>
                            ...
                            <button 
                              onClick={() => handleRenovacion(item)} 
                              className="p-2 text-emerald-500 hover:scale-110 transition-all" 
                              title="Renovar"
                            >
                              <RefreshCw size={16}/>
                            </button>
                            <button 
                              onClick={() => { if(window.confirm('¿Eliminar?')) supabase.from('proveedores').delete().eq('id', item.id).then(() => cargarTodo(session.user.id))}} 
                  ...
                  return (
                    <div key={i.id} className="bg-white/[0.02] border border-white/5 p-6 lg:p-8 rounded-[2rem] lg:rounded-[3rem] relative group flex flex-col h-full hover:border-white/10 transition-colors">
                      ...
                      <div className="bg-black/40 rounded-[1.5rem] lg:rounded-2xl p-4 mb-5 lg:mb-6 space-y-3 border border-white/5">
                        {i.nombre && (
                          <div className="flex justify-between items-center gap-2">
                            <div className="truncate flex-1"><p className="text-[8px] lg:text-[9px] text-slate-500 uppercase font-black tracking-widest">Nombre</p><p className="text-xs lg:text-sm font-bold text-slate-200 truncate" title={i.nombre}>{i.nombre}</p></div>
                            <button onClick={() => copiarAlPortapapeles(i.nombre)} className="text-slate-600 hover:text-white p-2 transition-colors flex-shrink-0"><Copy size={14}/></button>
                          </div>
                        )}
                        <div className="flex justify-between items-center gap-2">
                        ...
                        {i.url && (
                          <div className="flex justify-between items-center gap-2">
                            <div className="truncate flex-1"><p className="text-[8px] lg:text-[9px] text-slate-500 uppercase font-black tracking-widest">URL</p><p className="text-xs lg:text-sm font-bold text-slate-200 truncate" title={i.url}>{i.url}</p></div>
                            <button onClick={() => copiarAlPortapapeles(i.url)} className="text-slate-600 hover:text-white p-2 transition-colors flex-shrink-0"><Copy size={14}/></button>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between border-t border-white/5 pt-5 lg:pt-6 mt-auto">
                        <div className="space-y-1">
                            ...
                            {i.fecha_vencimiento && <div className="text-[8px] lg:text-[9px] font-bold text-slate-600 uppercase">{diasInv <= 0 ? "Venció" : `Faltan ${diasInv} d`}</div>}
                         </div>
                        
                        <div className="flex gap-1 lg:gap-2">
                          <button 
                            onClick={() => enviarWhatsAppProveedor(i)} 
                            className={`p-2 transition-colors ${!i.proveedor_whatsapp ? 'text-slate-700 cursor-not-allowed' : 'text-emerald-400 hover:text-emerald-300'}`} 
                            title="WhatsApp Proveedor"
                          >
                            <MessageCircle size={16}/>
                          </button>
                          <button 
                            onClick={() => abrirEdicionStock(i)} 
                          ...
                          {/* BOTON RENOVAR STOCK INCLUIDO AQUÍ */}
                          <button 
                            onClick={() => handleRenovacionStock(i)} 
                            className="text-emerald-500 hover:text-emerald-400 p-2 transition-colors hover:scale-110" 
                            title="Renovar (+1 Mes)"
                          >
                            <RefreshCw size={16}/>
                          </button>
                          ...
                          <button 
                            onClick={() => { if(window.confirm('¿Eliminar?')) supabase.from('inventario').delete().eq('id', i.id).then(() => cargarTodo(session.user.id))}} 
          ...
          <div className="animate-in fade-in duration-500 max-w-5xl mx-auto pb-20 lg:pb-0">
            ...
            <div className="space-y-3">
                {finanzasFiltradas.length === 0 ? (
                  <p className="text-center text-slate-500 text-xs md:text-sm mt-10 italic">No hay movimientos.</p>
                ) : finanzasFiltradas.map(f => (
                  <div key={f.id} className={`bg-white/[0.02] border border-white/5 p-4 lg:p-6 rounded-2xl lg:rounded-3xl flex justify-between items-center border-l-4 ${f.tipo === 'ingreso' ? 'border-l-emerald-500' : 'border-l-red-500'}`}>
                    <div className="flex gap-3 lg:gap-6 items-center italic">
                       <span className="text-slate-600 text-[8px] lg:text-[10px] font-black uppercase min-w-[65px] lg:min-w-[80px]">
                         {new Date(f.fecha).toLocaleDateString()}
                       </span>
                       <div>
                         <span className="text-white font-black uppercase text-xs lg:text-sm block">{f.titulo}</span>
                         <span className="text-[8px] lg:text-[10px] text-slate-500 uppercase font-bold">{f.servicio}</span>
                       </div>
                    </div>
        ...
        {vistaActual === 'ajustes' && (
          <div className="animate-in fade-in duration-500 max-w-4xl mx-auto pb-20 lg:pb-0">
            ...
            <div className="bg-white/[0.02] border border-white/10 p-6 lg:p-12 rounded-[2rem] lg:rounded-[3.5rem] space-y-8 lg:space-y-10">
                <div className="space-y-4">
                    <label className="text-[9px] lg:text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Nombre Administrador</label>
      ...
      {/* MODAL CLIENTE (CON SCROLL INTERNO PARA QUE NO DESAPAREZCA EL TÍTULO) */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl flex items-center justify-center p-4 lg:p-6 z-[70] animate-in zoom-in duration-300">
          <div className="bg-[#0a0a0f] border border-white/10 rounded-[2.5rem] lg:rounded-[3.5rem] w-full max-w-xl mx-2 sm:mx-4 max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            
            <div className="flex justify-between items-center p-6 lg:p-10 pb-4 lg:pb-6 border-b border-white/5 shrink-0 bg-[#0a0a0f] z-10">
              <h3 className="text-2xl lg:text-3xl font-black italic text-white uppercase tracking-tighter">{editId ? 'Editar' : 'Nuevo Cliente'}</h3>
              <button onClick={cerrarModal} className="text-slate-500 hover:text-white p-2 bg-white/5 rounded-full"><X size={20}/></button>
            </div>
            
            <div className="p-6 lg:p-10 pt-4 lg:pt-6 overflow-y-auto custom-scrollbar flex-1">
              <form onSubmit={handleGuardarCliente} className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                ...
                <input 
                  ...
                  className="sm:col-span-2 bg-white/5 border border-white/5 rounded-2xl p-4 lg:p-5 text-sm lg:text-base text-white font-bold outline-none" 
                  placeholder="Correo asociado (Opcional)" 
                />
                <input 
                ...
                {(form.servicio === 'IPTV' || form.servicio === 'Magis TV') && (
                  <input 
                    value={form.url} 
                ...
                <button 
                  type="submit" 
                  className="sm:col-span-2 bg-blue-600 py-5 lg:py-6 rounded-2xl font-black uppercase italic tracking-widest shadow-2xl mt-4 text-xs font-bold"
                >
                  GUARDAR REGISTRO
                </button>
              </form>
      ...
      {/* MODAL STOCK (CON SCROLL INTERNO PARA QUE NO DESAPAREZCA EL TÍTULO) */}
      {isInvModalOpen && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl flex items-center justify-center p-4 lg:p-6 z-[70] animate-in zoom-in duration-300">
          <div className="bg-[#0a0a0f] border border-white/10 rounded-[2.5rem] lg:rounded-[3.5rem] w-full max-w-xl mx-2 sm:mx-4 max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            
            <div className="flex justify-between items-center p-6 lg:p-10 pb-4 lg:pb-6 border-b border-white/5 shrink-0 bg-[#0a0a0f] z-10">
              <h3 className="text-2xl lg:text-3xl font-black italic text-white uppercase tracking-tighter">{editInvId ? 'Editar Cuenta' : 'Nueva Cuenta'}</h3>
              <button onClick={cerrarInvModal} className="text-slate-500 hover:text-white p-2 bg-white/5 rounded-full"><X size={20}/></button>
            </div>
            ...
            <div className="p-6 lg:p-10 pt-4 lg:pt-6 overflow-y-auto custom-scrollbar flex-1">
              <form onSubmit={handleGuardarStock} className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                ...
                {(invForm.servicio === 'IPTV' || invForm.servicio === 'Magis TV') && (
                  <>
                    <input 
                ...
                <div className="sm:col-span-2 border-t border-white/5 pt-5 mt-2">
                  <p className="text-[9px] font-black text-slate-500 ml-3 uppercase block mb-4">Proveedor (Opcional)</p>
                ...
                <button 
                  type="submit" 
                  className="sm:col-span-2 bg-white text-black py-5 lg:py-6 rounded-2xl font-black uppercase italic shadow-2xl mt-4 text-xs font-bold"
                >
                  GUARDAR CUENTA
                </button>
              </form>
