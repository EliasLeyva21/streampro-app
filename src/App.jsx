...
function App() {
  ...
  const verificarSuscripcion = useCallback(async (userId) => {
    try {
      const { data, error } = await supabase
        ...
        .eq('id', userId)
        .maybeSingle(); // Usar maybeSingle para evitar error 406 cuando no hay fila

      if (error) throw error;

      if (!data) {
        // No existe perfil -> cuenta no configurada, mostrar bloqueo
      ...
      if (!data.es_admin) {
        ...
        setSesionActiva(true); // Admin siempre activo
      }
    } catch (err) {
      console.error('Error verificando suscripción:', err.message);
      setSesionActiva(false); // En caso de error, bloquear acceso
    }
  }, []);
  ...
  const handleRenovacion = async (c) => {
    ...
    const { error } = await supabase
      .from('proveedores')
      .update({ fecha_vencimiento: f.toISOString().split('T')[0] })
      .eq('id', c.id);
    if (!error) {
      await supabase.from('ventas').insert([{
        cliente_id: c.id,
          ...
          <nav className="flex-1 space-y-3">
            {[
              ...
              <button
                ...
                className={`w-full flex items-center gap-4 p-4 rounded-2xl font-bold transition-all text-[13px] ${vistaActual === item.id ? 'bg-white/10 text-white border border-white/10' : 'text-slate-500 hover:text-white'} ${item.id === 'admin' ? 'border border-yellow-500/20 text-yellow-500/70 hover:text-yellow-400' : ''}`}
              >
            ...
            <header className="flex justify-between items-center mb-6 pt-4">
              ...
              <div className="flex gap-4">
                <div className="relative w-72">
                  ...
                  <input
                    type="text"
                    placeholder="Buscar nombre o fecha (AAAA-MM-DD)..."
                    className="w-full bg-white/5 border border-white/5 rounded-2xl py-3 pl-12 text-xs font-bold outline-none focus:border-white/20"
            ...
            <div className="flex gap-3 mb-10 overflow-x-auto pb-4 no-scrollbar">
              ...
              {['Todas', 'Netflix', 'Disney+', 'Spotify', 'Magis TV', 'Amazon Prime', 'HBO Max'].map(p => (
                <button
                  ...
                  className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all border ${plataformaFiltro === p ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white/5 border-white/5 text-slate-500 hover:border-white/20'}`}
                >
                  {p}
                </button>
              ))}
