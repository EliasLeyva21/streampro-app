    ...
    <div className="min-h-screen bg-[#050509] flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-sm animate-in fade-in zoom-in duration-500">
        <form onSubmit={handleLogin} className="bg-slate-950 p-10 rounded-[3rem] border border-slate-900 shadow-2xl flex flex-col items-center">
          ...
          <p className="text-slate-600 text-center text-[10px] font-bold mb-8 uppercase tracking-widest italic text-white/50">Inicia Sesión para Continuar</p>
          ...
          <a href="https://wa.me/519XXXXXXXX" target="_blank" className="flex items-center gap-2 text-[10px] font-black text-emerald-500 hover:text-emerald-400 uppercase tracking-widest transition-all italic">
...
function App() {
  ...
  const abrirEdicion = (cliente) => {
    ...
    setNombre(cliente.nombre_negocio);
    setWhatsapp(cliente.whatsapp);
    setServicio(cliente.servicio);
    setMonto(cliente.monto);
    setVencimiento(cliente.fecha_vencimiento);
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="min-h-screen bg-[#050509] text-slate-300 flex font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto relative bg-[#080811]">
        ...
        {vistaActual === 'dashboard' && (
          <div className="p-16 animate-in fade-in">
             <header className="mb-12 pt-2 flex justify-between items-center border-b border-slate-900 pb-8">
                ...
                <button onClick={() => setIsModalOpen(true)} className="bg-white text-black hover:bg-blue-600 hover:text-white px-8 py-4 rounded-2xl font-black uppercase text-xs flex items-center gap-2 transition-all">Nuevo Cliente</button>
             </header>
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-slate-950 border border-slate-900 p-8 rounded-[3rem]">
                   <p className="text-slate-500 text-[10px] font-black uppercase italic mb-8">Movimientos</p>
                   <div className="h-48"><ResponsiveContainer><AreaChart data={ventasHistoricas.slice(0,10).reverse()}><Area type="monotone" dataKey="monto" stroke="#3b82f6" fillOpacity={0.1} fill="#3b82f6" strokeWidth={4} /></AreaChart></ResponsiveContainer></div>
                </div>
                <div className="bg-slate-950 border border-slate-900 p-8 rounded-[3rem] border-l-4 border-l-emerald-500">
                    <p className="text-slate-500 text-[10px] font-black uppercase italic mb-1">Ingresos</p>
           ...
           <div className="p-16 animate-in fade-in">
              <header className="flex justify-between items-center mb-12 pt-2 border-b border-slate-900 pb-8">
                ...
                <div className="relative w-80">
                  ...
                  <input type="text" placeholder="Buscar..." className="w-full bg-slate-950 border border-slate-900 rounded-2xl py-4 pl-12 text-sm text-white font-bold italic outline-none" onChange={(e) => setFiltro(e.target.value)} />
                </div>
              </header>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {registros.filter(r => r.nombre_negocio.toLowerCase().includes(filtro.toLowerCase())).map(item => (
                  <div key={item.id} className="bg-slate-950 border border-slate-900 p-8 rounded-[2.5rem] flex flex-col gap-6">
                    <div className="flex justify-between">
                        <div>
                            <div className="font-bold text-slate-100 italic text-xl uppercase">{item.nombre_negocio}</div>
                            <div className="text-[10px] font-black text-blue-500 uppercase italic mt-1">{item.servicio}</div>
                        </div>
                    ...
                    <div className="flex items-center justify-between border-t border-slate-900 pt-6">
                       <div className={`text-xs font-black uppercase italic ${esAtrasado(item.fecha_vencimiento) ? 'text-red-500' : 'text-emerald-500'}`}>
                       ...
                       <div className="flex gap-2">
                          ...
                          <button onClick={() => eliminarRegistro(item.id)} className="p-2 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 hover:text-white"><Trash2 size={14}/></button>
                       </div>
                    </div>
                  </div>
                ))}
        ...
        {vistaActual === 'stock' && (
           <div className="p-16 animate-in fade-in">
              <h2 className="text-4xl font-black text-white italic uppercase mb-12 pt-2 border-b border-slate-900 pb-8">Inventario</h2>
              <div className="bg-slate-950 border border-slate-900 rounded-[3rem] overflow-hidden">
                <table className="w-full text-left">
                  <thead className="bg-black text-[10px] font-black uppercase text-slate-500 italic">
                    <tr><th className="p-6">Producto/Cuenta</th><th className="p-6">Stock</th><th className="p-6">Estado</th></tr>
                  </thead>
                  <tbody className="divide-y divide-slate-900 font-bold italic text-sm">
                    {inventario.length > 0 ? inventario.map(i => (
                      <tr key={i.id} className="hover:bg-slate-900/50">
                        <td className="p-6 text-white uppercase">{i.nombre}</td>
                        <td className="p-6">{i.cantidad}</td>
                        <td className="p-6"><span className="text-emerald-500 bg-emerald-500/10 px-3 py-1 rounded-full text-[10px]">ACTIVO</span></td>
                      </tr>
                    )) : <tr><td colSpan="3" className="p-12 text-center text-slate-700">Sin stock registrado</td></tr>}
                  </tbody>
                </table>
              </div>
           </div>
        ...
        {vistaActual === 'finanzas' && (
           <div className="p-16 animate-in fade-in">
              <h2 className="text-4xl font-black text-white italic uppercase mb-12 pt-2 border-b border-slate-900 pb-8">Ventas Realizadas</h2>
              <div className="space-y-4">
                {ventasHistoricas.length > 0 ? ventasHistoricas.map(v => (
                  <div key={v.id} className="bg-slate-950 border border-slate-900 p-6 rounded-3xl flex justify-between items-center">
                    <div className="flex gap-6 items-center">
                       <div className="text-slate-600 text-xs font-black italic">{new Date(v.fecha_pago).toLocaleDateString()}</div>
                       <div className="text-white font-black italic uppercase">{v.proveedores?.nombre_negocio || 'Cliente'}</div>
                       <div className="bg-slate-900 px-4 py-1 rounded-full text-[10px] font-black text-slate-400 uppercase italic">{v.metodo_pago}</div>
                    </div>
                    <div className="text-emerald-500 font-black italic">S/. {v.monto?.toFixed(2)}</div>
                  </div>
                )) : <p className="text-center p-12 text-slate-700 font-bold italic">No hay historial de ventas</p>}
              </div>
           </div>
        )}
            ...
            <div className="bg-slate-950 border border-slate-900 p-10 rounded-[3rem] space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2 col-span-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase italic tracking-widest ml-2">Correo Electrónico (Solo Lectura)</label>
                  ...
                  <div className="space-y-2 col-span-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase italic tracking-widest ml-2">Contraseña del Sistema (Solo Lectura)</label>
                    <div className="relative">
                      <Key className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-700" size={18}/>
                      <input type="password" readOnly value="********" className="w-full bg-black/40 border border-slate-900 rounded-2xl py-4 pl-14 text-sm font-bold text-slate-500 cursor-not-allowed outline-none" />
                    </div>
                  </div>
                </div>
                <div className="p-6 bg-blue-900/10 border border-blue-900/20 rounded-2xl">
                    <p className="text-[10px] font-bold text-blue-400 italic flex items-center gap-2"><AlertCircle size={14}/> Los datos de acceso están vinculados a Supabase y no pueden ser modificados desde aquí por seguridad.</p>
                </div>
            </div>
          </div>
        )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl flex items-center justify-center p-6 z-50">
          <div className="bg-slate-950 border border-slate-800 p-12 rounded-[4rem] w-full max-w-lg animate-in zoom-in">
            <div className="flex justify-between items-center mb-8"><h3 className="text-3xl font-black italic text-white uppercase">{editId ? 'Editar' : 'Registro'}</h3><button onClick={cerrarModal} className="text-slate-500 hover:text-white"><X size={24}/></button></div>
            <form onSubmit={handleGuardarCliente} className="grid grid-cols-2 gap-5">
              <input required value={nombre} onChange={e => setNombre(e.target.value)} className="col-span-2 bg-black border border-slate-800 rounded-3xl p-5 text-white font-bold" placeholder="Nombre Cliente" />
              <input required value={whatsapp} onChange={e => setWhatsapp(e.target.value)} className="col-span-2 bg-black border border-slate-800 rounded-3xl p-5 text-white font-bold" placeholder="WhatsApp" />
              <select required value={servicio} onChange={e => setServicio(e.target.value)} className="bg-black border border-slate-800 rounded-3xl p-5 text-white font-bold outline-none">
                <option value="">Servicio...</option><option value="Netflix">Netflix</option><option value="Disney+">Disney+</option><option value="Spotify">Spotify</option>
              </select>
              ...
              <div className="col-span-2"><label className="text-[10px] font-black text-slate-600 ml-4 uppercase italic">Vencimiento</label><input required type="date" value={vencimiento} onChange={e => setVencimiento(e.target.value)} className="w-full bg-black border border-slate-800 rounded-3xl p-5 text-slate-500 font-bold" /></div>
              <button type="submit" className="col-span-2 bg-blue-600 hover:bg-blue-500 py-6 rounded-3xl font-black uppercase italic shadow-lg">Guardar Cambios</button>
            </form>
          </div>
        </div>
      )}
